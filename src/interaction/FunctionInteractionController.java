...
public class FunctionInteractionController extends PlotInteractionController{
    
    public FunctionInteractionController(GraphElementManager plotManager, ComputationCoordinator coordinator) {
    ...
    public void updateSelection(double mouseX, double mouseY, EvaluationContext context, ArrayList<Intersection> intersections, Viewport viewport){
        ...
        selectedCurve = curve;
        if(!Double.isFinite(mouseX) && !Double.isFinite(mouseY)) return;
        selectedPoint = applySnapping(
            selectedCurve.targettedPoint(mouseX, mouseY, context),
            selectedCurve,
            intersections,
            viewport
        );
